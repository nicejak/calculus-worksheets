...
\ExamClass{Math 242}
\ExamName{Classwork 11, \S 11.9, 11.10}
\ExamHead{Fall 2024}
\fancyfoot{}
...
\begin{document}
    ...
    \begin{enumerate}
        \item For the following functions, find a series representation and the radius of convergence.
        \begin{enumerate}
            \item $f(x)=\dfrac{1}{1+4x^{2}}$\vfill\vfill
            \item $g(x)=\dfrac{x^{3}-5x^{2}+6x}{x-2}$\vfill
        \end{enumerate}
        \newpage
        \item Compute the Maclaurin series for the following functions.
        \begin{enumerate}
            \item $f(x)=(2+x)^{5}$\vfill
            \newpage
            \item $g(x)=\ln\left|\dfrac{1+x}{1-x}\right|$\vfill
            \newpage
        \end{enumerate}
        \item Compute the Taylor series at $a=1$ for the function $f(x)=x^{5}-5x^{4}+27x^{2}-3x+17$\vfill
    \end{enumerate}
